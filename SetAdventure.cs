...
public class SetAdventure
{
    ...
    //Incremented whenever you edit an object
    public static int numObjectsEdited = 1;
    ...
    public partial class Display : Form
    {
        public Display()
        {
            ...
            // Organize submenu
            MenuItem[] menusOrganize = new MenuItem[11];
            menusOrganize[0]  = new MenuItem("by ID");
            menusOrganize[1]  = new MenuItem("by recently edited");
            menusOrganize[2]  = new MenuItem("by ID, X Rotation");
            menusOrganize[3]  = new MenuItem("by ID, Y Rotation");
            menusOrganize[4]  = new MenuItem("by ID, Z Rotation");
            menusOrganize[5]  = new MenuItem("by ID, X");
            menusOrganize[6]  = new MenuItem("by ID, Y");
            menusOrganize[7]  = new MenuItem("by ID, Z");
            menusOrganize[8]  = new MenuItem("by ID, Variable 1");
            menusOrganize[9]  = new MenuItem("by ID, Variable 2");
            menusOrganize[10] = new MenuItem("by ID, Variable 3");
            
            menusOrganize[0] .Click += eventOrganizeID;
            menusOrganize[1] .Click += eventOrganizeRecent;
            menusOrganize[2] .Click += eventOrganizeIDXRot;
            menusOrganize[3] .Click += eventOrganizeIDYRot;
            menusOrganize[4] .Click += eventOrganizeIDZRot;
            menusOrganize[5] .Click += eventOrganizeIDX;
            menusOrganize[6] .Click += eventOrganizeIDY;
            menusOrganize[7] .Click += eventOrganizeIDZ;
            menusOrganize[8] .Click += eventOrganizeIDVar1;
            menusOrganize[9] .Click += eventOrganizeIDVar2;
            menusOrganize[10].Click += eventOrganizeIDVar3;
            ...
            // Help submenu
            MenuItem[] menusHelp = new MenuItem[1];
            menusHelp[0] = new MenuItem("Version");
            
            menusHelp[0].Click += eventHelpVersion;
            
            MenuItem menuItemHelp = new MenuItem("&Help", menusHelp);
        ...
        public void eventHelpVersion(object sender, EventArgs e)
        {
            MessageBox.Show("Set Adventure 1.2");
        }
        ...
        public void eventEditAddBlankObject(object sender, EventArgs e)
        {
            ...
            blank.var2 = 0;
            blank.var3 = 0;
            blank.lastEdited = numObjectsEdited;
            numObjectsEdited++;
            blank.genDisp();
        ...
        public void eventOrganizeRecent(object sender, EventArgs e)
        {
            organizeType = 10;
            objList.Sort();
            updateObjectList();
        ...
        public void eventClickApplyButton(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    currObj.lastEdited = numObjectsEdited;
                    numObjectsEdited++;
                    
                    currObj.genDisp();

                    //System.Drawing.Point scollPosition = lstBox.AutoScrollOffset; //save current scroll position for later
                    
                    Display.updateObjectList();

                    lstBox.SelectedIndex = lstBox.Items.Count-1; //select last object to scroll down...
                    lstBox.SelectedItem = currObj; //...and then scroll back up so the object is at the top

                    //lstBox.AutoScrollOffset = scollPosition; //restore scroll position. didnt end up actually scrolling. useless...
                }
                catch
                {
    ...
    public class SETObject : IComparable
    {
        ...
        public int index; //index in the main list of objects. used for stable sorting
        public int lastEdited; //changed every time this item gets edited
        ...
        public int CompareTo(object obj)
        {
            ...
            //Recent edited overrides everything
            if (organizeType == 10)
            {
                //Order by last edited
                if (orderToCompare.lastEdited < lastEdited)
                {
                    return -1;
                }
                else if (orderToCompare.lastEdited > lastEdited)
                {
                    return 1;
                }

                //If theres a tie, keep it the same order
